...
def generate_daily_report_html_email(
    ...
    no_issues_html = (
        ''.join(site_sections)
        if site_sections
        else '<div class="no-issues"><h3>No issues found for the reporting period</h3></div>'
    )
    ...
    <body style="margin:0; padding:20px; background-color:#ffffff; color:#111111;">
        ...
        <div class="footer">
            ...
            <p>For support, contact:
                <a href="mailto:vortexai.dashboard@vortex.vivotek.com">
                    vortexai.dashboard@vortex.vivotek.com
                </a>
            </p>
        </div>
    </body>
...
def _generate_site_section(site: str, issues: List[DailyReportRow], start_date: datetime, end_date: datetime) -> str:
    ...
    for issue in issues:
        ...
        issue_link = (
            f'<a href="https://vivotek.atlassian.net/browse/{clean_issue_key}" '
            f'target="_blank" class="jira-link">{clean_issue_key}</a>'
        )
        table_rows.append(
            f"""
            <tr>
