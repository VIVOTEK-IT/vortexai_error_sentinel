    ...
    try:
        ...
        if config.email.recipients:
            ...
            email_sent = email_service.send_daily_report_email(
                html_content=test_html, subject="[TEST] VortexAI Error Monitoring System - Email Test"
            )
...
def test_html_email_template():
    ...
    try:
        ...
        mock_site_reports = {"stage": {"issues": [mock_issues[0]]}, "prod": {"issues": [mock_issues[1]]}}
        ...
        html_content = generate_daily_report_html_email(
            site_reports=mock_site_reports, start_date=start_date, end_date=end_date, total_issues=2
        )
